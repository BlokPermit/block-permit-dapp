...
const ProjectPage = ({project}: InferGetServerSidePropsType<typeof getServerSideProps>) => {
    ...
    return (
        <div className="px-40 mb-10">
            <BreadCrumbs/>
            <ProgressBar className="my-16" actualPhase={2} selectedPhase={selectedPhase}
                         handlePhaseChange={(phase: number) => setSelectedPhase(phase)}/>
            <div className="flex justify-between mb-10">
                ...
                <div className="flex items-center gap-2">
                    <IconButton className="text-white bg-main-200 hover:text-main-200 hover:bg-white"
                                icon={<FaPaperclip/>} text={"Attachments"}
                                onClick={() => setIsAttachmentsPopupOpen(true)}/>
                    <DocumentDropdown documentId={project.dppUrl ?? ""} documentType="dpp" isPresent={isDPPPresent}
                                      onDocumentChange={onDocumentChange} fileName={project.dppUrl}/>
                </div>
            </div>
            <div className="grid grid-cols-8 gap-12 border-b border-gray-900/10 mb-10">
                <div className="col-span-3 pb-12">
                    ...
                    <IconCard icon={<FaFileContract/>} title="Impact on Environment?"
                              value={project.constructionImpactsEnvironment ? "Yes" : "No"}/>
                </div>
            ...
            {isAttachmentsPopupOpen &&
                <AttachmentsPopup opinionProviderId={0} onClose={() => setIsAttachmentsPopupOpen(false)}/>}
            <RoleBasedComponent
                projectManagerComponent={
                    <div className="overflow-x-auto">
                        ...
                        <IconButton text={numOfSelected > 0 ? "Send Selected" : "Send All"} icon={<FaArrowUp/>}
                                    onClick={handleSend}/>
                    </div>
                }
            />
                    ...
                    <div>
                        ...
                        <div className="flex justify-end gap-4">
                            <IconButton className="mt-3 bg-white text-main-200 hover:bg-main-200 hover:text-white"
                                        text="Add Attachments" icon={<FaPaperclip/>}
                                        onClick={() => setIsAttachmentsPopupOpen(true)}/>
